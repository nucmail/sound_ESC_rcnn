...
from tensorflow.keras.layers import Conv2D, BatchNormalization, Activation, GlobalAveragePooling2D, Reshape, GlobalMaxPooling2D
...
def model_resnet(num_classes,input_shape =[128,None,6], num_filters =24,wd=1e-3):
    ...
    Split2=  Lambda(My_freq_split2)(inputw)
    ...
    for stack in range(4):
        for res_block in range(num_res_blocks):
            ...
            '''
            residual1 = Conv2D(1, kernel_size = (1, 1), padding = 'same')(ResidualPath1)
            residual1 = BatchNormalization(axis = 3)(residual1)
            cbam1 = cbam_block(residual1)
            ResidualPath1 = keras.layers.add([ResidualPath1, residual1, cbam1])
            
            residual2 = Conv2D(1, kernel_size = (1, 1), padding = 'same')(ResidualPath2)
            residual2 = BatchNormalization(axis = 3)(residual2)
            cbam2 = cbam_block(residual2)
            ResidualPath2 = keras.layers.add([ResidualPath2, residual2, cbam2])
            '''
    ...
    #residual2 = Conv2D(1, kernel_size = (1, 1), padding = 'same')(ResidualPath)
    #residual2 = BatchNormalization(axis = 3)(residual2)
    #cbam2 = cbam_block(residual2)
    #ResidualPath = keras.layers.add([ResidualPath, residual2, cbam2])
