...
class LR_WarmRestart(keras.callbacks.Callback):
    ...
    def __init__(self,nbatch,initial_lr,min_lr,epochs_restart,Tmult):
        ...
        self.startEP=0
        self.Tmult=Tmult
#    在每次迭代开始时，检查迭代次数，判断是否达到了epochs_restart中设定的学习率改变的迭代处，    
...
class MixupGenerator():
    def __init__(self, X_train, y_train, batch_size=32, alpha=0.2, shuffle=True, crop_length=150): #datagen=None):
...
#生成0-indexes的随机数组
    ...
    def __data_generation(self, batch_ids):
        ...
        for j in range(X1.shape[0]):
            self.NewLength = random.randint(141,187)
